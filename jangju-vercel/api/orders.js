    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;border:1px solid #e0c88a;border-radius:12px;overflow:hidden;">
      ...
      <div style="padding:28px;background:#FDF6E3;">
        ...
        <table style="width:100%;border-collapse:collapse;">
          <tr style="border-bottom:1px solid #e0c88a;"><td style="padding:10px 8px;color:#888;font-weight:700;width:120px;">👤 Name</td><td style="padding:10px 8px;font-weight:600;">${order.name}</td></tr>
          <tr style="border-bottom:1px solid #e0c88a;"><td style="padding:10px 8px;color:#888;font-weight:700;">📞 Phone</td><td style="padding:10px 8px;font-weight:600;">${order.phone}</td></tr>
          <tr style="border-bottom:1px solid #e0c88a;"><td style="padding:10px 8px;color:#888;font-weight:700;">📍 Address</td><td style="padding:10px 8px;font-weight:600;">${order.address}</td></tr>
          <tr style="border-bottom:1px solid #e0c88a;"><td style="padding:10px 8px;color:#888;font-weight:700;">🛒 Quantity</td><td style="padding:10px 8px;font-weight:600;">${order.quantity || '—'}</td></tr>
          <tr style="border-bottom:1px solid #e0c88a;"><td style="padding:10px 8px;color:#888;font-weight:700;">💰 Amount</td><td style="padding:10px 8px;font-weight:800;color:#3D0C02;">${order.totalAmount || '—'}</td></tr>
          <tr style="border-bottom:1px solid #e0c88a;"><td style="padding:10px 8px;color:#888;font-weight:700;">💳 Payment</td><td style="padding:10px 8px;font-weight:600;">${order.payment || '—'}</td></tr>
          ${order.message ? `<tr><td style="padding:10px 8px;color:#888;font-weight:700;">💬 Message</td><td style="padding:10px 8px;font-style:italic;">${order.message}</td></tr>` : ''}
        </table>
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'DELETE') {
    if (!id) return res.json({ success: false, error: 'No order ID provided' });
    try {
